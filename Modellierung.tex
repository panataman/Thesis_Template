...
\begin{description}
	...
	\begin{description}
		...
		\item[count\_number\_day] Der aktuelle Wert von der Laufvariable
		\textbf{number\_days}.
		\item[Q\_losses] Wert der aktuellen Verlustk\"altemengeals
...
Abschnitten vorgenommen\todo{Abschnitt einfuegen!}.
...
stehenden K\"alteleistung\todo{Bei Caro nachgucken, ob das leistung ist!}, zu
Temperaturschwankungen. Die engen Grenzen des zul\"assigen Temperaturbandes
d\"urfen nicht \"uberschritten werden.
...
Windleistungsdaten der Vattenfall-Regelzone \todo{was ist das heute?} aus dem
...
\end{align}
\begin{align*}
	Q_{v_{ln}} &= \begin{cases}
	1h \cdot \aptranslog, & \text{au\ss erhalb der \"Offnungszeiten}\\
	1h \cdot (\aptranslog + \pmehr), & \text{innerhalb der \"Offnungszeiten}
	\end{cases}
\end{align*}

\begin{description}[\dth]
	\item[$Q_{v_{ln}}$] Verlustk\"altemenge in $kJ$
...
Wärmeenergie ist\todo{die nachfolgende Gleichung geh\"ort nicht dahin! oder?}.

\begin{equation}
	\emax = m \cdot c \cdot (t(i) - t_{max}) + Q_v
\label{emax}
\end{equation}
...
werden alle K\"uhlstellen gefunden, deren Kapazit\"at\todo{was ist das?} noch nicht ausgesch\"opft
