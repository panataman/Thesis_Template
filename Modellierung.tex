...
\textbf{Refrigerator}\todo{trennung!} explizit erkl\"art.
...
\begin{description}
	...
	Werten aus der elementeweisen\todo{trennung} Multiplikation des
	Vektors mit den Werten der spezifischen W\"armedurchgangskoeffizienten
	mit dem Vektor mit Werten f\"ur die Durchgangsfl\"achen (vgl.
	\cref{vmod} auf der Seite \pageref{vmod}).
...
\begin{description}
	...
	\begin{description}
		...
		Vektors mit Werten der spezifischen\todo{trennung}
		W\"armedurchgangskoeffizienten mit dem Vektor mit den Werten f\"ur
		die Durchgangsfl\"achen (vgl. \cref{vmod} auf der Seite
		\pageref{vmod}).
	...
	Ber\"ucksichtigung der zus\"atzlichen\todo{trennung} Verluste sichergestellt. Der
	zweite R\"uckgabewert ist die Zeitspanne bis zum Erreichen der kritischen
	maximalen Temperatur (vgl. \cref{tauk} auf der Seite \pageref{tauk}).
...
\cref{fig:klassendiagramm} durch eine Linienverbindung/Komposition verdeutlicht.
Auf der Seite des Ganzen endet die Linie mit einem ausgef\"ullten Rhombus. Das
komplette Modell ist nach diesem Prinzip aufgestellt. Das kleinste Element des
Ganzen stellt die K\"alteanlage (\textbf{Refrigerator}) dar.
...
stehenden K\"alteleistung, zu Temperaturschwankungen\todo{trennung}. Die engen Grenzen des
...
muss. Ist der st\"undliche Bedarf gr\"o\ss er als 15\,\% der Prognose, wird die
...
\begin{description}[\dth]
	...
	\item[$\aptranslog(i)$] logarithmierte Mittelwert der
...
\begin{description}[\dth]
	...
	\item[$k$] Wärmedurchgangskoeffiziente in $\mathrm{\frac{W}{m^2 \cdot
	K}}$
	\item[$t_{max}$] obere Temperaturgrenze in $ ^{\circ} $C
	\item[$t(i)$] Temperatur zur Stunde $i$ in $ ^{\circ} $C
\end{description}
...
zeigt den Ablauf beim "'Auff\"uhlen"' der K\"uhlstellen  f\"ur genau einen
...
zu der jeweiligen K\"alteeinheit. ergibt sich dadurch, dass \todo{das mache ich
noch heute}
\end{description}
...
\cref{pload} auf der Seite \pageref{pload} zugrunde, die in der
Funktion \textbf{power\_for\_load\_net()} implementiert
ist.
...
Netz geladen, sodass sie genau eine Stunde \"uberstehen, ohne die obere
Temperaturgrenze zu verletzen. Der Vorgang endet anschlie\ss end, und es kann
zum n\"achsten Zeitschritt der Simulation \"ubergegangen werden.
...
Unterschiede aufweisen. In der Konfigurationsdatei
...
sind. Die Gruppierung der einzelnen Kühleinheiten zu einem Modellsupermarkt
erfolgt in der \textbf{config\_supermarkets.m$\,$}-Datei durch Definition eines
oder mehrerer $i\times2\,$-Cell ($i\in \mathbb{Z}^+_0$) Arrays, die jeweils ein
Supermarkt abbilden. Jede Zeile eines solchen Cell Arrays ist für je eine Art
Kühleinheit reserviert. In die erste Spalte kommt der Name der K\"uhlzelle,
deren Eigenschaften in der \textbf{config\_fridges.m}-Datei (vgl.
